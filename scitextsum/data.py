  ...
  def __init__(self, vocab_file, max_size):
    ...
    with codecs.open(vocab_file, 'r', encoding='utf-8') as vocab_f:
      for line in vocab_f:
        idx += 1
        pieces = line.split()
        if len(pieces) != 2 or not isascii(line):
          bad_lines += 1
          sys.stderr.write('Bad line, or non-unicode at line %d: %s' % (idx, line))
          continue
        if pieces[0] in [SENTENCE_START, SENTENCE_END, UNKNOWN_TOKEN, PAD_TOKEN, START_DECODING, STOP_DECODING]:
          raise Exception('<S>, </S>, [UNK], [PAD], [START] and [STOP] shouldn\'t be in the vocab file, but %s is' % w)
        if pieces[0] in self._word_to_id:
          raise ValueError('Duplicated word: %s.' % pieces[0])
          print('Dupicated word ignored: %s' % pieces[0])
        self._word_to_id[pieces[0]] = self._count
        self._id_to_word[self._count] = pieces[0]
        self._count += 1
        if self._count > max_size:
          print('Max size: {:d}, stopping reading'.format(max_size))
...
def get_word_ids(text, vocab, pad_len=None, pad_id=None):
...
def snippet_gen(text, start_tok, end_tok, inclusive=True):
...
def to_sentences(paragraph, include_token=True):
